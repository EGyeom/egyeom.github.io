...
// 3번 학생이 2번 학생이나 4번 학생에게 체육복을 빌려주면 학생 4명이 체육수업을 들을 수 있습니다.
    ...
    //         reserveSize--;
    //         answer++;
    //         if(lostSize * reserveSize == 0) break;
    //     }
    //     else
    //     {
    //         lostIndex++;
    //         if(lostIndex >= lostSize)
    //         {
    //             lostIndex = 0;
    //             reserveIndex++;
    ...
    //     }
    // }
    // while(lostSize * reserveSize != 0)
    // {
    //     int temp = lost[lostIndex] - reserve[0];
    //     temp = abs(temp);
    //    // printf("%d %d %d %d %d %d\n", lost[lostIndex], lostIndex, reserve[0], 0, lostSize, reserveSize);
    //    // printf("%d\n", temp);
    //     if(temp <= 1)
    //     {
    //         lost.erase(lost.begin() + lostIndex);
...
int solution(int n, vector<int> lost, vector<int> reserve) {
    ...
    sort(lost.begin(), lost.end());
    sort(reserve.begin(), reserve.end());
    vector<int> templost = lost;
    vector<int> tempreserve = reserve;

    for(int i =0; i < lost.size(); i++)
    {
        for(int j = 0; j < tempreserve.size(); j++)
        {
            if(lost[i] == tempreserve[j])
            {
                tempreserve.erase(tempreserve.begin() + j);
                answer++;
                break;
            }
        }
    ...
    for(int i =0; i < reserve.size(); i++)
    {
        ...
        {
            if(reserve[i] == templost[j])
            {
                templost.erase(templost.begin() + j);
                break;
            }
        }
    }
